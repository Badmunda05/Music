...
class Leaderboard:
    ...
    async def generate(self, bot_details: dict) -> str:
        ...
        text = f"**🎉 ᴛᴏᴘ 10 ᴜsᴇʀs ᴏꜰ {bot_details['mention']}**\n\n"
    ...
    async def broadcast(self, hellbot, text, buttons):
        ...
        to_log = f"**ʟᴇᴀᴅᴇʀʙᴏᴀʀᴅ ᴀᴜᴛᴏ ʙʀᴏᴀᴅᴄᴀsᴛ ᴄᴏᴍᴘʟᴇᴛᴇᴅ ɪɴ** `{time_taken}`\n\n**ᴛᴏᴛᴀʟ ᴄʜᴀᴛs:** `{count}`\n**sᴜᴄᴄᴇss:** `{success}`\n**ꜰᴀɪʟᴇᴅ:** `{failed}`\n\n**🧡 ᴇɴᴊᴏʏ sᴛʀᴇᴀᴍɪɴɢ! ʜᴀᴠᴇ ꜰᴜɴ!**"
    ...
    async def send_message(self, hellbot, buttons, chat: int, text: str):
        ...
        try:
            ...
            return 2, f"{chat} -:- ᴄʜᴀᴛ ɪᴅ ɪɴᴠᴀʟɪᴅ\n"

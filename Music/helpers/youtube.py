    ...
    def _parse_html(self, response):
        ...
        for video in videos:
            ...
            if "videoRenderer" in video.keys():
                ...
                # Handle publish_date with fallback
                ...
                try:
                    ...
                    if publish_date:
                        res["publish_time"] = publish_date.strftime(
                            ...
                            'th' if 4 <= publish_date.day <= 20 or 24 <= publish_date.day <= 30 else ['st', 'nd', 'rd'][(publish_date.day - 1) % 10 % 3]
                        )
                    else:
                        res["publish_time"] = "Unknown"
                        LOGS.warning(f"Publish date not available for video: {video_url}")
                except Exception as e:
                    res["publish_time"] = "Unknown"
                    LOGS.error(f"Failed to fetch publish date for {video_url}: {e}")
